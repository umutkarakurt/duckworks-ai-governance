...
| FRIA-03 | Fairness validation              | Data & AI + HR + Risk            | Test relevant group performance/outcome disparities using lawful, appropriate methodology and synthetic/test data before production; establish escalation thresholds. | Validation plan/report; methodology; issue log         | Synthetic DT-02 test demonstrated; blocking remains |
...
| EG-06  | Fairness / subgroup performance                        | Synthetic matched-pair DT-02 testing now demonstrates control/test logic; no real-applicant or production fairness validation exists. | Data & AI / HR                   | Complete lawful, methodologically justified production fairness testing for the intended applicant population. | Open - Critical |
...
### 10.1 Related synthetic operating evidence — DT-02

The operating-evidence package at [`../../80-operating-evidence/AI-005-ducktalent/`](../../80-operating-evidence/AI-005-ducktalent/) demonstrates a synthetic implementation of `DT-02 — Pre-Deployment Fairness & Adverse-Impact Testing`.

The worked test uses 24 synthetic applicants in 12 matched pairs and a deliberately unapproved `Career_Gap_Months` scoring penalty. The control detects the feature-governance violation, measures its effect, blocks the pre-deployment gate, records the exception, removes the feature, and retests the full synthetic population.
...
This evidence improves the design and synthetic testability of the FRIA fairness-validation requirement, but `EG-06` remains **Open — Critical**. No real applicant or protected-characteristic data are used; no legal discrimination or rights-compliance conclusion is supported; production job relevance, lawful fairness-data methodology, accessibility, human oversight, vendor evidence, and remedy remain unvalidated.

The FRIA decision therefore remains **DO NOT DEPLOY**.
